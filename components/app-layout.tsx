...
export function AppLayout({ children, title, subtitle }: AppLayoutProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      {/* 1. GLOBAL NAVIGATION & HEADER: Compact on Mobile (~52px), Clean 3-Column on Desktop */}
      <header className="sticky top-0 z-40 bg-card/95 backdrop-blur-md border-b border-border/80 px-3 sm:px-6 py-2 md:py-3 shadow-2xs">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 md:gap-4">
          ...
          {/* Center: Inline Page Title for Mobile (< md) */}
          {mainTitle && (
            <div className="block md:hidden flex-1 min-w-0 mx-1.5 text-center sm:text-left">
              <span className="text-xs font-extrabold text-foreground truncate block leading-tight">
                {mainTitle}
              </span>
              {secondarySubtitle && (
                <span className="text-[10px] font-semibold text-muted-foreground truncate block leading-tight">
                  {secondarySubtitle}
                </span>
              )}
            </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-2 pt-3 border-t border-border space-y-2.5 animate-in slide-in-from-top-2 duration-200">
            ...
            {/* Mobile User Profile & Logout inside Drawer */}
      ...
      <main className="flex-1 w-full">
        {mainTitle && (
          <div className="hidden md:block border-b border-border/80 bg-card/50 backdrop-blur-sm px-4 sm:px-6 py-5 shadow-2xs">
