      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 grid grid-cols-2 md:grid-cols-3 items-center gap-4">
        ...
        <div className="flex items-center justify-start">
          <Link href="/dashboard" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-extrabold text-primary-hover dark:text-primary uppercase tracking-wider">Enterprise ERP</span>
            </div>
          </Link>
        </div>
              ...
              <Link
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 ${
                  isActive
                    ...
                    : item.highlight
                    ? 'bg-primary text-primary-foreground hover:bg-primary-hover hover:text-primary shadow-sm'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
                }`}
              >
                {item.highlight && <span className="w-1.5 h-1.5 rounded-full bg-primary-foreground animate-pulse" />}
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <div className="relative">
            ...
            >
              <Globe className="w-3.5 h-3.5 text-primary-hover dark:text-primary" />
            ...
            {langDropdownOpen && (
              <div className="absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-lg border border-slate-200 py-1 z-50 animate-in fade-in-50 zoom-in-95 duration-100">
                <button
                  ...
                  className={`w-full px-3 py-2 text-left text-xs font-bold flex items-center justify-between hover:bg-slate-50 ${currentLang === 'KH' ? 'text-primary-hover dark:text-primary bg-primary/20' : 'text-slate-700'}`}
                >
                  <span>🇰🇭 Khmer</span>
                  {currentLang === 'KH' && <span className="w-1.5 h-1.5 rounded-full bg-primary" />}
                </button>
                <button
                  onClick={() => { setCurrentLang('EN'); setLangDropdownOpen(false); }}
                  className={`w-full px-3 py-2 text-left text-xs font-bold flex items-center justify-between hover:bg-slate-50 ${currentLang === 'EN' ? 'text-primary-hover dark:text-primary bg-primary/20' : 'text-slate-700'}`}
                >
                  <span>🇬🇧 English</span>
                  {currentLang === 'EN' && <span className="w-1.5 h-1.5 rounded-full bg-primary" />}
                </button>
              </div>
            )}
          </div>
